...
$tableNews = new FieldFrm("fndatabase",$tablename,$_FN['datadir'],$_FN['lang'],$_FN['languages']);
$tableArguments = new FieldFrm("fndatabase","{$tablename}_arguments",$_FN['datadir'],$_FN['lang'],$_FN['languages']);
...
if (count($AllnewsByArguments) > 0)
{
	...
	foreach ($AllnewsByArguments as $argumentid=> $items)
	{
		...
		$href = FN_RewriteLink("index.php?mod={$_FN['mod']}&amp;op=$argumentid$my");
...
if ($argument != "")
	echo "<div class=\"newsarchive_back\" ><a href=\"".FN_RewriteLink("index.php?mod={$_FN['mod']}")."\">&lt;&lt;&nbsp;".FN_i18n("back")."</a></div>";
/**
 * Print arguments
 * @global array $_FN
...
function FN_NARC_PrintByDate($all)
{
	...
	{
		$href = FN_RewriteLink("index.php?mod={$_FN['mod']}&amp;yy=$year&amp;op=$argument");
		...
		if ($yy != "")
		{
			...
			{
				$href = FN_RewriteLink("index.php?mod={$_FN['mod']}&amp;yy=$year&amp;mm=$month&amp;op=$argument");
...
function FN_NARC_PrintBar($tablename)
{
	...
	echo "<a href=\"".FN_RewriteLink("index.php?mod={$_FN['mod']}")."\">".$_FN['sectionvalues']["title"]."</a>";
	if ($yy != "")
	{
		echo "&gt;&gt;<a href=\"".FN_RewriteLink("index.php?mod={$_FN['mod']}&amp;yy=$yy")."\">$yy</a>";
		if ($mm != "")
		{
			echo "&gt;&gt;<a href=\"".FN_RewriteLink("index.php?mod={$_FN['mod']}&amp;yy=$yy&amp;mm=$mm")."\">".$_FN['months'][$mm - 1]."</a>";
		}
	}
	if ($argument != "")
	{
		...
		$argumentvalues = $tableArguments->GetRecordTranslated($argumentvalues);
		$title = $argumentvalues['title'];
		echo "&gt;&gt;<a href=\"".FN_RewriteLink("index.php?mod={$_FN['mod']}&amp;op=$argument")."\">$title</a>";
	}
}

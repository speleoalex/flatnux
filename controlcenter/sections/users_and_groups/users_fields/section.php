...
if (isset($invert_to['position']) && isset($invert_from['position']))
{
	...
	$metaform->xmltable->UpdateRecord($invert_from);
	$metaform->xmltable->UpdateRecord($invert_to);
}
$records = $metaform->xmltable->GetRecords(false,false,false,"position");
//-----------------change position---------------------------------------------<
...
function edit_struct_table($tablename,$path,$databasename)
{
	...
	FN_xmltableeditor(false,"tables",$params);
	//dprint_xml(file_get_contents($tparams['datafile']));
	return;
}
 ...
 *
  ...
  if (isset($_POST['body']))
  {
  $xml=FN_GetParam("body",$_POST,"flat");
  //dprint_r($xml);
  $tableusers =  new FieldFrm("{$_FN['database']}",array("xml"=>$xml),$_FN['database'],$_FN['datadir']);
  $formvals=$tableusers->formvals;
  //dprint_r($formvals);
  $errors=false;
  if (!isset($formvals['username']))
  ...
  echo "<br /><br /><br /><a href=\"javascript:history.back();\">&lt;&lt; ".  FN_Translate("back")."</a>";
  }
  else
  {
  FN_Write("{$_FN['datadir']}/{$_FN['database']}/fn_users.php",$file);
  echo FN_Translate("the data were successfully updated");
  echo "<br /><br /><br /><a href=\"?opt=$opt\">".  FN_Translate("next")." &gt;&gt;</a>";
  }
  }
  ...
  {
  FN_EditContent("{$_FN['datadir']}/{$_FN['database']}/fn_users.php");
  }
*/

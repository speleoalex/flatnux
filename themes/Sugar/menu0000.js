...
/*
		if (menu.isSubMenu){
			try{
				if (blnNetscape6){
					left+=(getLayer(menu.name).offsetWidth - 4);
				}else{
					left+=(getLayer(menu.name).clientWidth - 8);
				}
			}catch(e){
			}
		}else{
...
function closeSubMenus(handle){
	...
	if (menuStack.length > 0){
		for (var i = menuStack.length-1; i >=0; i--){
			...
			if (menu.id == handle.getAttribute('name')){
				currentMenu = menu;
				break;
			}else{
				closeMenu(menu);
...
*/
	if (menu){
		...
		//menu.y = top;
		if (isSubMenu){
			if (menu.getAttribute('name') == currentMenu.getAttribute('name')){
				//menuStack.pop();
				menuPop();
				setMenuVisibility(currentMenu, false);
			}
...
/*
	var name = menu.name;
	while (name){
		parent = getMenu(menu.paneID, name);
		if (parent){
			parent.visible = on;

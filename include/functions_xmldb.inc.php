...
class xmldbfrm_field_localfile
{
    ...
    function show($params)
    {
        ...
        $onclick="tmp = window.open('filemanager.php?{$mime}dir={$fmpath}&local=1&mode=t&filemanager_editor=local&opener=$idop','filemanager','toolbar= 0,location= 0,directories= 0,status= 0,menubar= 0,scrollbars= 1,resizable= 1,width=640,height=480');tmp.focus();return false;";
    ...
    function view($params)
    {
        ...
        if (is_dir($params['value']))
            $html.="<iframe style=\"border:0px;height:400px;width:500px\" src=\"filemanager.php?dir=$val&mime=all\">$val</iframe>";
        else

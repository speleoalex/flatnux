...
function FN_HtmlAdminOnOff()
{
	...
	{
		$stout .= "<div id=\"fn_adminonoff\" style=\"z-index:99999;height:auto;width:auto\" class=\"fn_admin\">";
		...
		if (file_exists("controlcenter.php"))
		{
			...
			$stout .= "<img style=\"cursor:pointer\" title=\"".FN_Translate("control center")."\"  onclick=\"window.location='{$_FN['siteurl']}controlcenter.php'\"  alt=\"".FN_Translate("control center")."\" style=\"border:0px;vertical-align:middle\" src=\"".FN_FromTheme("images/configure.png")."\" />";
			//$stout .= "</a>";
		}
...
function FN_HtmlAdminOptions()
{
	...
	if ($_FN['fneditmode'] && FN_CanModify($_FN['user'],"sections/{$_FN['mod']}"))
	{
		...
		if (FN_UserCanEditFolder("sections/{$_FN['mod']}"))
		{
			...
			{
				$html .= "<tr><td><img src=\"images/modify.png\" alt=\"\"/></td><td><button onclick=\"window.location='index.php?mod={$_FN['mod']}&opt=fnc_ccnf_section_{$_FN['mod']}'\">".
...
";
			...
			//new section------------------------------------------------------<
			$html .= "<button onclick=\"filemanagerpopup=window.open('filemanager.php?mime=all&amp;dir=sections/{$_FN['mod']}','filemanager_','top=10,left=10,height=450,width=480,scrollbars=no');filemanagerpopup.focus()\" >".FN_Translate("filemanager")."</button>";
		}
		$html .= "</div>";
	}
	return $html;

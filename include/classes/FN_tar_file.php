 ...
 * 
 */
class FN_tar_file extends FN_archive
{
    function __construct($name)
    {
        $this->FN_archive($name);
    ...
    function create_tar()
    {
        ...
        foreach ($this->files as $current)
        {
            ...
            if (false != ($fp = @ fopen($current['name'], "rb")))
            {
                $this->add_data($block);
                while (false != ($temp = fread($fp, 1048576))) $this->add_data($temp);
    ...
    function extract_files()
    {
        $pwd = getcwd();
        chdir($this->options['basedir']);
        if (false != ($fp = $this->open_archive()))
        {
            if ($this->options['inmemory'] == 1)
                $this->files = array();
            while (false != ($block = fread($fp, 512)))
            {
                $temp = unpack("a100name/a8mode/a8uid/a8gid/a12size/a12mtime/a8checksum/a1type/a100symlink/a6magic/a2temp/a32temp/a32temp/a8temp/a8temp/a155prefix/a12temp", $block);
                $file = array('name' => $temp['prefix'] . $temp['name'],
                    'stat' => array(2 => $temp['mode'], 4 => octdec($temp['uid']), 5 => octdec($temp['gid']), 7 => octdec($temp['size']), 9 => octdec($temp['mtime'])), 'checksum' => octdec($temp['checksum']),
                    'type' => $temp['type'], 'magic' => $temp['magic']);
                ...
                if (false != ($new = @ fopen($file['name'], "wb")))
                {
